...
def load_token():
    ...
    if len(t) < 8:
        ...
        try:
            ...
            pass
    # also drop an easy-to-copy text file right next to this script
    try:
        here = os.path.dirname(os.path.abspath(__file__))
        with open(os.path.join(here, "riri-token.txt"), "w") as f:
            f.write(t + "\n")
    except OSError:
        pass
    return t
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/ping"):
            self._json(200, {"ok": True, "name": "riri-agent", "version": VERSION, "hint": TOKEN[:4]})
        else:
    ...
    def do_POST(self):
        if not self.path.startswith("/run"):
            self._json(404, {"ok": False, "err": "not found"})
            return
        tok = (self.headers.get("X-Riri-Token") or "").strip().upper()
